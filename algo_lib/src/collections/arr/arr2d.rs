...
#[derive(Clone, Eq, PartialEq, Debug, Default, Hash, Ord, PartialOrd)]
pub struct Arr2d<T> {
    d1: usize,
    d2: usize,
    data: Vec<T>,
...
impl<T> Arr2d<T> {
    ...
    pub fn rotate_clockwise(self) -> Self {
        unsafe {
            ...
            for (id, el) in self.into_iter().enumerate() {
                let (i, j) = (id / d2, id % d2);
                let ptr: *mut T = (*res.as_mut_ptr()).as_mut_ptr();
                ptr.add(j * d1 + d1 - i - 1).write(el);
            }
