...
based on FIFO (First In First Out) principle, and the last position is
...
impl MyCircularQueue {
    fn new(k: i32) -> Self {
        ...
        Self {
            capacity: Cell::new(k as usize),
            head: Cell::new(0),
            tail: Cell::new(0),
            size: Cell::new(0),
            buffer: RefCell::new(buf),
        }
    }
    ...
    fn en_queue(&mut self, value: i32) -> bool {
        ...
        self.tail.set((tail + 1) % self.capacity.get());
    ...
    fn de_queue(&self) -> bool {
        ...
        self.head.set((head + 1) % self.capacity.get());
        self.size.set(self.size.get() - 1);
        true
    }
    ...
    fn rear(&self) -> i32 {
        ...
        let tail = self.tail.get();
        let idx = (tail + capacity - 1) % capacity;
        let buffer = self.buffer.borrow();
        buffer[idx]
    }
    ...
    fn is_full(&self) -> bool {
        self.capacity.get() == self.size.get()
    }
}
