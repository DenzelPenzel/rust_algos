...
The cost of a binary string s is defined as the sum of 
all indices i (0-based) such that s[i] == '1'.
...
impl Solution {
    pub fn generate_valid_strings(n: i32, k: i32) -> Vec<String> {
        fn dfs(i: usize, n: usize, k: i32, cost: i32, prev_one: bool, path: &mut String, ans: &mut Vec<String>) {
            if i == n {
                ans.push(path.clone());
                return;
            }
            ...
            path.push('0');
            dfs(i + 1, n, k, cost, false, path, ans);
            path.pop();

            if !prev_one && cost + i as i32 <= k {
                path.push('1');
                dfs(i + 1, n, k, cost + i as i32, true, path, ans);
                path.pop();
            }
        }
        ...
        dfs(0, n as usize, k, 0, false, &mut path, &mut res);
