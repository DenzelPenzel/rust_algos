...
An integer k is called sortable if k divides n and you can 
sort nums in non-decreasing order by sequentially performing 
the following operations:

Partition nums into consecutive subarrays of length k.
Cyclically rotate each subarray independently any number of 
times to the left or to the right.
...
impl Solution {
    pub fn sortable_integers(nums: Vec<i32>) -> i32 {
        ...
        for k in divisors {
            let is_valid = nums.chunks(k)
                .zip(sorted_nums.chunks(k))
                .all(|(orig, sort)| Self::is_rotation_clever(orig, sort));
    ...
    fn is_rotation_clever(original: &[i32], sorted: &[i32]) -> bool {
       let doubled = sorted.repeat(2);
        doubled.windows(original.len()).any(|w| w == original)
    }
}
